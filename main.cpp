...
template<typename E> void multiMrg(std::string fileName, int buffSize, int numRuns);
...
void multiMrg(std::string inFile, std::string outFile, int buffSize, int numRuns)	//Temporarily using quicksort for testing
{
	...
	if (ifs.is_open())
	{
		...
		{
			inBuff[i] = firstElement(inFile, i + 1, runStart[i], buffSize);
		}

		//write smallest value to file
		writeData(outFile, smallestElement(arr, size), 1);
		runStart[]++;
...
template<typename E>
E smallestElement(E arr[], int size)	//returns smallest element in array
{
	E smallest = arr[0];
